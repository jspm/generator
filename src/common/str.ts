...
export class Replacer {
  ...
  remove (start: number, end: number, trimWs: boolean | RegExp = false) {
    ...
    if (trimWs) {
      if (typeof trimWs === 'boolean')
        trimWs = wsRegEx;
      const endIndex = this.idx(end);
      const [wsMatch] = this.source.slice(endIndex).match(trimWs) ?? [];
      this.source = this.source.slice(0, endIndex) + this.source.slice(endIndex + wsMatch?.length ?? 0);
...
function addOffset (offsetTable: [number, number][], idx: number, offset: number) {
  ...
  while (i-- > 0) {
    ...
    if (offsetIdx < idx || offsetIdx === idx && (eq = true))
      break;
  }
  if (eq)
    offsetTable.splice(i, 1, [idx, offset + offsetTable[i][1]]);
  else
    offsetTable.splice(i + 1, 0, [idx, offset]);
}
